...
test_that("Test CV returns", {
  ...
  res2 <- cv(br, toyml, base.algorithm="RANDOM", cv.folds=3, cv.results = TRUE)
...
test_that("Test CV seed", {
  res1 <- cv(dbr, toyml, base.algorithm="RANDOM", cv.folds=4, cv.seed = 123)
  res2 <- cv(dbr, toyml, base.algorithm="RANDOM", cv.folds=4, cv.seed = 123)
  expect_equal(res1, res2)
  set.seed(123)
  res3 <- cv(dbr, toyml, base.algorithm="RANDOM", cv.folds=4)
  expect_equal(res1, res3)

  res4 <- cv(dbr, toyml, base.algorithm="RANDOM", cv.folds=4,
