...
*Binary Relevance* multi-label method with the base algorithm *Random Forest*^[Requires the 
...
The transformation methods require a base algorithm (binary or multi-class) and use their
predictions to compose the multi-label result. In the **utiml** package there are some
default base algorithms that are accepted, but if you need another you can easily 
developement your own^[see the section *Create a new base Learner*].

Each base algorithm requires a specific package, you need to install manually this packages,
...
for more details^[Base algorithm `J48` do not work very well with multicore].
